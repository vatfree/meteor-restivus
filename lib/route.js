...
Route.prototype._resolveEndpoints = function () {
    ...
    _.each(this.endpoints, function (endpoint, method, endpoints) {
        // Only process HTTP method endpoints, not route-level options like authRequired, roleRequired
        if (!_.contains(availableMethods, method)) {
            return;
        }
        
        if (_.isFunction(endpoint)) {
            // Convert function to object with action property
            return endpoints[method] = {
                action: endpoint
            };
        }
        // If it's already an object, leave it as-is
        // It should already have the proper structure (action, authRequired, etc.)
    });
};
...
Route.prototype._configureEndpoints = function () {
    _.each(this.endpoints, (endpoint, method) => {
        // Only process HTTP method endpoints, not route-level options
        if (!_.contains(availableMethods, method)) {
            return;
        }
...
Route.prototype._callEndpoint = async function (endpointContext, endpoint) {
    let auth = await this._authAccepted(endpointContext, endpoint);
    ...
    if (auth.success) {
        if (await this._roleAccepted(endpointContext, endpoint)) {
            ...
            return await DDP._CurrentInvocation.withValue(invocation, async function () {
                ...
                // Handle async actions that return Promises (Meteor 3 compatibility)
 ...
 */
Route.prototype._authAccepted = async function (endpointContext, endpoint) {
...
/*
  ...
  @returns An object of the following format:
    ...
    will contain the auth data when successful and an optional error response when auth fails.
 */
Route.prototype._authenticate = async function (endpointContext) {
...
/*
  ...
  @returns True if the authenticated user belongs to <i>any</i> of the acceptable roles on the
           endpoint
 */
Route.prototype._roleAccepted = async function (endpointContext, endpoint) {
    if (endpoint.roleRequired) {
        ...
        if (_.isString(rolesRequired) || _.isArray(rolesRequired)) {
            return await Roles.userIsInRoleAsync(endpointContext.user._id, rolesRequired);
        } else if (_.isObject(rolesRequired)) {
            check(rolesRequired, {
                roles: Match.OneOf(String, Array),
                scope: Match.Optional(String)
            });
            return await Roles.userIsInRoleAsync(endpointContext.user._id, rolesRequired.roles, rolesRequired.scope);
        }
